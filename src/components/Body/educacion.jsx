import '../../css/educacion.css'
export const Educacion = () =>{
    return(
        <>
            <div className="contenedor" id='educacion'>
                <div className="subContenedor">
                    <p className='subtitle'>Educación</p>
                    <div className='ctn-item ctn-educacion'>
                        <p className='subsubtitle'>Universidad e Institutos</p>
                        <div className='cursos'>
                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href="./assets/doc/estudios/certus.pdf">
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/certus.png" alt="Certus" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-title'>Certus</p>
                                            <p className='item-subtitle'>Desarrollador Web Front End</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                72 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                2023
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/bachiller.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/upn.png" alt="Certus" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-title'>Universidad Privada el Norte</p>
                                            <p className='item-subtitle'>Bachiller en Ingeniería de Sistemas</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/birrete.svg" alt="reloj" />
                                                09/05/2024
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/egresado.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/upn.png" alt="Certus" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-title'>Universidad Privada el Norte</p>
                                            <p className='item-subtitle'>Bachiller en Ingeniería de Sistemas</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="reloj" />
                                                2017 - 2022
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/birrete.svg" alt="reloj" />
                                                16/12/2023
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>
                        </div>
                    </div>
                    <div className='ctn-item ctn-cursos'>
                        <p className='subsubtitle'>Cursos</p>
                        <div className='cursos'>

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/platzi-diploma-frontend-developer.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/platzi.png" alt="platzi" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>FrontEnd Developer</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                15 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                04-2024
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            {/* <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/platzi-diploma-html-css.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/platzi.png" alt="platzi" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>HTML y CSS</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                23 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                05-2024
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a> */}

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/platzi-diploma-transformaciones-transiciones-css.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/platzi.png" alt="platzi" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>Transformaciones y transiciones en CSS</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                18 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                05-2024
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/platzi-diploma-mobile-first.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/platzi.png" alt="platzi" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>Responsive Design</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                25 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                05-2024
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            {/* <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/platzi-diploma-sass.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/platzi.png" alt="platzi" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>Fundamentos SASS</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                26 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                05-2024
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a> */}

                            {/* <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/JavaScript.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/ftm.png" alt="movistar" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>Programación con JavaScript</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                40 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                04-2022
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/Wordspress.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/ftm.png" alt="movistar" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>WordPress</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                30 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                04-2022
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a>

                            <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/htmlCss.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/ftm.png" alt="movistar" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>Diseño Web con HTML5 + CSS</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                30 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                04-2022
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a> */}

                            {/* <a className='item-card-e' data-aos="fade-up" data-aos-anchor-placement="top-bottom" target='_blank' href='./assets/doc/estudios/OFFICE-INTERMEDIO.pdf'>
                                <div className='card-e'>
                                    <div className='item-img'>
                                        <img src="./assets/img/estudios/ftm.png" alt="movistar" />
                                    </div>
                                    <div className='ctn-item-txt'>
                                        <div className='item-txt'>
                                            <p className='item-subtitle'>Office Intermedio</p>
                                        </div>
                                        
                                        <div className='ctn-item-data'>
                                            <div className='item-data'>
                                                <img src="./assets/svg/reloj.svg" alt="reloj" />
                                                25 Horas
                                            </div>
                                            <div className='item-data'>
                                                <img src="./assets/svg/calendario.svg" alt="calendario" />
                                                04-2022
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </a> */}

                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}